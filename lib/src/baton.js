...
const db = r.db('baton')
const batons = db.table('batons')
const teams = db.table('teams')
...
export function drop(label: String, notify: Boolean = false) {
  return new Promise((resolve, reject) => {
    batons.filter({id}).delete().run((err, body) => {
      if (!err) {
    ...
    if (teamId && teamDomain) {
      teams.filter({teamId, teamDomain}).limit(1).run(dbConn, (err, cursor) => {
        if (!err) {
          cursor.toArray()
            .then(team => {
              console.log('DERP TEAM', team[0])

              resolve(team[0].token)
            })
...
export function register(token: String, teamId: String, teamDomain: String) {
  console.log('Registering team member... ', token, teamId, teamDomain)
