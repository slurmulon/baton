...
// returns all batons in the database
export function all() {
  return new Promise((resolve, reject) => {
...
// returns all batons in the database for a specific team
export function byTeam({teamId, teamDomain}) {
  return new Promise((resolve, reject) => {
    batons.filter({team_id: teamId, team_domain: teamDomain}).run(dbConn, runPromise.all(resolve, reject))
  })
}
...
// provides a flattened array of tags across all batons
...
// provides a flattened array of tags across a team's batons
export function allTeamTags({teamId, teamDomain}) {
  return new Promise((resolve, reject) => {
    batons
      .filter({team_id: teamId, team_domain: teamDomain})
      .hasFields('tags')
      .concatMap(btn => btn('tags'))
      .distinct()
      .run(dbConn, runPromise.all(resolve, reject))
...
// finds all batons in the database that match all of the provided tags
export function byTags(tags: String) {
  return new Promise((resolve, reject) => {
    if (tags) {
      batons
        .filter(baton => baton('tags').contains(...tags))
        .run(dbConn, runPromise.all(resolve, reject))
    } else {
      reject(`Tag(s) must be provided: ${tags}`)
    }
  })
}
...
// finds all batons in the database that match tags for a team
export function byTeamTags(tags: String, teamId: String, teamDomain: String) {
  return new Promise((resolve, reject) => {
    if (tags) {
      batons
        .filter({team_id: teamId, team_domain: teamDomain})
...
// finds all batons in the database that match a tag for a team
export function byTeamTag({tag, teamId, teamDomain}) {
  return byTeamTags([tag], teamId, teamDomain)
}
