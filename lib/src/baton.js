...
export function search({label: String, tags: Array}) {
  return new Promise((resolve, reject) => {
    table.filter(baton => {
      return baton('tags').contains(...tags) || baton('label').eq(label)
    }).run(dbConn, (err, batons) => {
      if (err) {
        console.log(`[baton] Failed to search batons ${err}`)
        reject(err)
      }
