...
export const api = (app: Object) => {
  ...
  app.all('/v1/slack/*', (req, res, next) => {
    ...
    // const command = (body.command || req.body.command).replace('/', '')
    const method  = req.method.toLowerCase()

    console.log('[baton:slack:api:DEBUG] Incoming slack request', command, req.body)

    console.log('UNTOUCHED COMMAND', req.body.command)

    if (command &&
        command in commands &&
        (req.body.command === '/baton' || method in commands[command])) {
      // const method = command !== 'baton' ? req.method.toLowerCase() : Object.keys(command[command])[0]
