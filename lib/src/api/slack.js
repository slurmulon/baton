...
export const api = (app: Object) => {
  ...
  app.all('/v1/slack/*', (req, res, next) => {
    ...
    if (command &&
      ...
      next(slack.err(req, `Unsupported command ${command} ${method.toUpperCase()}`))
    }
  })
  ...
  const commands = {
    ...
    find: {
      'get': auth.required((req, res, next) => {
        ...
        if (tags) {
          return baton.byTeamTags(tags, creds(req).teamId, creds(req).teamDomain)
            .then(btns => res.send(slack.items(req, btns)))
            .catch(error(req, res, next))
        } else {
          res.status(400).send(slack.err(req, `One or more tag is required: ${req.body.tags}`))
        }
      })
    },
    ...
    tags: {
      'get': auth.required((req, res, next) => {
        return baton.allTeamTags(creds(req))
    ...
    help: {
      'get': (req, res) => {
        ...
        if (!req.body.text) {
          ...
          Object.keys(guide).filter(cmd => cmd != 'error').forEach(cmd => {
            allCmds += ` * ${cmd}: ${guide[cmd]}\n`
          })
